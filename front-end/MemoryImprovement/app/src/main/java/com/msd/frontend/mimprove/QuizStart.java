...
//        corrctAnswers.clear();
    ...
    private JSONObject castInputMapToObject() throws JSONException
    {
        JSONObject jsonObject = new JSONObject();
        JSONArray questionsArray = new JSONArray();
        for (Map.Entry<String, ArrayList<String>> answersMap : mapOfAnswerQuestions.entrySet())
        {
            ...
            currentQuestion.put("text",answersMap.getKey());
            ArrayList<String> temp = answersMap.getValue();
            currentQuestion.put("user_answer",temp.get(0));
...
//        corrctAnswers.clear();
    ...
    private void saveInputQuiz(final ProgressDialog progressDialog,JSONObject objectOfAnswers) throws JSONException, UnsupportedEncodingException {
        android.util.Log.e("Save Input Quiz call", "yes");
...
//        callParams.put("username", "test_patient");
        ...
        clientHandler.post(QuizStart.this, "http://54.172.172.152/quiz/save_info", entity, "application/json", new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                Log.e("Response quiz success", new String(responseBody));
...
//        callParams.put("username", "test_patient");
        ...
        clientHandler.post(QuizStart.this,"http://54.172.172.152/quiz/save_quiz", entity, "application/json",new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                Log.e("Input quiz success", new String(responseBody));
...
//                }
            ...
            @Override
            public void onFailure(int statusCode, Header[] headers, byte[] responseBody, Throwable error) {
                Log.e("Response InputQuiz fail", new String(responseBody)+"\t"+statusCode);
                progressDialog.dismiss();
//                progressDialog.dismiss();
//                Toast.makeText(getApplicationContext(),"Sorry there was some problem",Toast.LENGTH_SHORT).show();
            }
